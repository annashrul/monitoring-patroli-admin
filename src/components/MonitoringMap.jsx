...
export default function MonitoringMap({ site, sites, posts, satpamLocations = {}, statusLabels, height = "min(520px, 70vh)" }) {
            ...
            <Popup>
              <div className="text-sm font-sans">
                ...
                <div className="mt-1">
                  ...
                  {post.status === 'green' ? (statusLabels?.green?.label || 'Aman') :
                   post.status === 'yellow' ? (statusLabels?.yellow?.label || 'Scan Ulang') : (statusLabels?.red?.label || 'Belum')}
                  </Badge>
                </div>
        ...
        {Object.values(satpamLocations).map((loc) => (
          <Marker
            ...
            icon={L.divIcon({
              ...
              html: `<div style="display:flex;flex-direction:column;align-items:center;"><div style="width:16px;height:16px;background:${loc.color || '#3B82F6'};border:3px solid #fff;border-radius:50%;box-shadow:0 0 6px ${loc.color || '#3B82F6'}60;"></div><div style="font-size:10px;font-weight:700;color:#fff;background:rgba(0,0,0,0.7);padding:1px 5px;border-radius:3px;white-space:nowrap;margin-top:2px;">${loc.name}</div></div>`,
              iconSize: [16, 16],
              iconAnchor: [8, 8],
            })}
          />
